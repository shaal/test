...
(function ($) {
  ...
  Drupal.facets.makeChosenDropdown = function (context, settings) {
    ...
    $('.js-facets-chosen-links').once('facets-dropdown-transform').each(function () {
      var ul = $(this);
      var active_facets = ul.data('activeFacets');
      ...
      if (active_facets.length > 0) {
        active_facets = active_facets.split(";");
      }
      else {
        active_facets = [];
      }
      ...
      dropdown.on('change', function(evt, params){
        if (typeof params.selected !== 'undefined') {
          $(this).trigger('facets_filter', [params.selected]);
        }
        else if (typeof params.deselected !== 'undefined') {
          $(this).trigger('facets_filter', [params.deselected]);
        }
      });
